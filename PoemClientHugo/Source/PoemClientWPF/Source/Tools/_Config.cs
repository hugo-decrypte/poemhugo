...
namespace PoemClientWPF.Tools
{
    public class Config
    {
        ...
        private readonly IniFile iniFile = null;
        public bool cancel = false;
        private static bool isConfigChosen = false;
        public string GetCurrentEnv()
        {
            if (iniFile != null)
                return iniFile.GetCurrentEnv();
        ...
        private Config(string path)
        {
            ...
            var savedConfig = iniFile.GetConfigs()["Save"];
            try
            {
                if (iniFile.GetConfigs()["Save"] != "")
                {
                    iniFile.SetCurrent(savedConfig);
                    InitializeConfigFile((this.configPath = savedConfig));
                    return;
                }
                if (isConfigChosen == false)
                {
                    if (new SelectConfig(iniFile).ShowDialog() == true)
                    {
                        ...
                        {
                            MessageBox.Show("Invalid config file : " + ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                            Process.GetCurrentProcess().Kill();
                        }

                        this.configPath = selectedFilePath.Replace(GetCurrentEnv(), "");
                        this.cancel = false;
        ...
        private void InitializeConfigFile(string pathDir)
        {
            ...
            {
                pathDir = pathDir.Remove(0, GetCurrentEnv().Length);
        ...
        {
            InitializeConfigFile(GetCurrentEnv() + this.configPath);
        ...
        public string GetKeyValue(string key)
        {
            try
            {
                ...
                else
                {
                    ...
                    MessageBox.Show(iniFile.GetConfigs()["TAG_MissingKey"] + " \"" + key + "\"\n(" + tmp + ")", iniFile.GetConfigs()["TAG_ConfigError"], MessageBoxButton.OK, MessageBoxImage.Error);
            ...
            {
                MessageBox.Show(iniFile.GetConfigs()["TAG_BootError"] + " : " + configPath.ToString().Split('\\')[configPath.ToString().Split('\\').Length - 1] + "\n" + "Line " + ExtractExceptionDetail(ex.Message).ToString().Split(' ')[2], iniFile.GetConfigs()["TAG_ConfigError"], MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        {
            configuration.Save();
            Console.WriteLine("Path: " + GetCurrentEnv() + configPath);
